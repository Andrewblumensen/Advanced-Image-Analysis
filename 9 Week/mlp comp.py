...
def simple_forward(x,W):
    z = np.c_[x,np.ones((x.shape[0]))]@W[0]
    h1 = np.maximum(z,0)
    z = np.c_[z,np.ones((z.shape[0]))]@W[1]
    h2 = np.maximum(z,0)
    yh = np.c_[h2,np.ones((z.shape[0]))]@W[2]
    y = np.exp(yh)/np.sum(np.exp(yh),axis=1,keepdims=True)
    return y,h1, h2
...
def simple_backward(x, W, t, learning_rate=0.1):
    y, h1, h2 = simple_forward(x,W)
    L= -np.sum(t*np.log(y + 10e-10))/x.shape[0]
    # print(L)
    d2 = y - t
    q2 = np.c_[h2,np.ones((x.shape[0]))].T@d2/y.shape[0]
    d1 = (h2>0)*(d2@W[2].T)[:,:-1]
...
def simple_init_weights(n):
    W = []
    W.append(np.random.randn(785,n)*np.sqrt(2/3))
    W.append(np.random.randn(n+1,n+1)*np.sqrt(2/(n+1)))
    W.append(np.random.randn(n+2,10)*np.sqrt(2/(n+1)))
    return W




W = simple_init_weights(400)
...
for i in range(0,n_iter):
    ...
    # yh = yh/np.max(yh)
    y = np.exp(yh)/np.sum(np.exp(yh),axis=1,keepdims=True)
    L= -np.sum(t[0:4000,:]*np.log(y + 10e-10))/x.shape[0]
    # print(L)
    d2 = y - t[0:4000,:]
    q2 = np.c_[h2,np.ones((x.shape[0]))].T@d2/y.shape[0]
    d1 = (h2>0)*(d2@W[2].T)[:,:-1]
...
z = np.c_[a,np.ones((a.shape[0]))]@W[0]
h1 = np.maximum(z,0)
z = np.c_[z,np.ones((z.shape[0]))]@W[1]
h2 = np.maximum(z,0)
yh = np.c_[h2,np.ones((z.shape[0]))]@W[2]
...
correctm = (guess == gt[4001:g].T)

correct = np.sum(correctm)/(g-4001)
